...
if(isset($_POST['register'])) {
    ...
    // $bcryptPass = password_hash($userPassword, PASSWORD_BCRYPT);
    $checkQuery = "SELECT * FROM `users` WHERE `userEmail` = '$userEmail'";
    $checkRes = mysqli_query($conn,$checkQuery);
    if (mysqli_num_rows($checkRes) > 0) {

        $sameEmail = "This email was already registered. Please choose a different email.";
    ...
    if($userName == "" || $userContact == "" || $userEmail == "" ||$userPassword == "") {
        ...
        <strong>Please fill all the forms fields</strong>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
      </div>';
    } else {
        $query = "INSERT INTO `users` (`userName`, `userContact`, `userEmail`, `userPassword`, `regiterAt`)
 VALUES ('$userName', '$userContact', '$userEmail', '$userPassword', current_timestamp())";
                            ...
                            <form action="signup.php" class="login-form" method="post">
                                <div class="account__login register">
                                    <div class="account__login--header mb-25">
                                        ...
                                        <p class="account__login--header__desc"><a href="login.php"> If you already have an account? Log In now </a></p>
                                    </div>
                                    
                                    <div class="account__login--inner">
                                        <label>
                                            <input class="account__login--input" name="name" value="<?php echo @$_POST['name']?>" placeholder="Username" type="text">
                                        </label>
                                        <label>
                                            <input class="account__login--input" name="contact" value="<?php echo @$_POST['contact']?>" placeholder="Phone Number" type="tel" >
                                            
                                        </label>
                                        <label>
                                            <input class="account__login--input" name="email" value="<?php echo @$_POST['email']?>" placeholder="Email Addres" type="email">
                                            <p class="text-danger"><?php echo @$sameEmail?></p>
                                        </label>
                                        <label>
                                            <input class="account__login--input" name="password" value="<?php echo @$_POST['password']?>" placeholder="Password" type="password">
                                        </label>
                                        
                                        <button class="account__login--btn primary__btn mb-10" name="register" type="submit">Submit & Register</button>
