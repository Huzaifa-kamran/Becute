...
$filter = $_POST['productStatus'];

if($filter == 1){
    
$products = "SELECT * FROM `products` INNER JOIN `categories` ON `products`.`catID` = `categories`.
`catID` WHERE `proStatus` = '1' AND `inStock` = '1'";

}elseif($filter == 0){

    $products = "SELECT * FROM `products` INNER JOIN `categories` ON `products`.`catID` = `categories`.
`catID` WHERE `proStatus` = '0'";

}elseif($filter == 2){

    $products = "SELECT * FROM `products` INNER JOIN `categories` ON `products`.`catID` = `categories`.
    `catID` WHERE `proStatus` = '1' AND `inStock` = '0'";
}else{
