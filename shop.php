...
if(isset($_GET['catID'])){
    $catID = $_GET['catID'];
    $products = "SELECT * FROM  `products` WHERE `inStock` = 1 AND `proStatus` = 1 AND `catID` = '$catID'";
}else{

    $products = "SELECT * FROM  `products` WHERE `inStock` = 1 AND `proStatus` = 1 LIMIT $limit OFFSET $offset";
}
                        ...
                        <div class="swiper-slide">
                            <div class="shop__collection--card text-center">
                                <a class="shop__collection--link" href="shop.php?catID=<?php echo $row['catID'] ?>">
                                    ...
                                    <h3 class="shop__collection--title mb-0"><?php echo $row['catName'] ?></h3>
                                </a>
                            </div>
                        </div>
                        <?php
                        ...
                        <div class="shop__sidebar--widget widget__area d-none d-lg-block">
                            <div class="single__widget widget__bg">
                                ...
                                <?php 
                                    ...
                                    $catListRes = mysqli_query($conn,$catList);
                                    if(mysqli_num_rows($catListRes) > 0){
                                      while($row = mysqli_fetch_assoc($catListRes)){
                                        $catID = $row['catID'];
                                          ?>
                                           <li class="widget__categories--menu__list">
                                        <label class="widget__categories--menu__label d-flex align-items-center">
                                            <img class="widget__categories--menu__img" src="admin/assets/images/icons/<?php echo $row['catImg']?>" alt="categories-img">
                                            <span class="widget__categories--menu__text"><?php echo $row['catName']?></span>
                                        ...
                                        <ul class="widget__categories--sub__menu">
                                            <?php 
                                            $productList = "SELECT * FROM `products` WHERE `catID` = '".$catID."'";
                                            $productListRes = mysqli_query($conn,$productList);
                                            if(mysqli_num_rows($productListRes)){
                                              while($row2 = mysqli_fetch_assoc($productListRes)){

                                                  ?>

                                             <li class="widget__categories--sub__menu--list">
                                                <a class="widget__categories--sub__menu--link d-flex align-items-center" href="product-details.php?proID=<?php echo $row2['proID']?>">
                                                    <img class="widget__categories--sub__menu--img" src="admin/assets/images/products/<?php echo $row2['img1']?>" alt="categories-img">
                                                    <span class="widget__categories--sub__menu--text"><?php echo $row2['proName']?></span>
                                                </a>
                                            </li>
                  ...
                  <li>';
                     ...
                     if (mysqli_num_rows($res2) > 0) {
                       ...
                       for ($i = 1; $i <= $totalPages; $i++) {
                         ...
                         echo '<li class="pagination__list'.$activeClass.'"><a href="shop.php?page='.$i.'" class="pagination__item link">'.$i.'</a></li>';
                       }
                     }
                     if($totalPages>$page){
                      echo '<li class="pagination__list">
